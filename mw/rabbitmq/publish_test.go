...
func Test_publish(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			_ = publishInternal(&p, "foo", c.retryCount, "100", nil, c.input)
		})
	}
}
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			for i := 0; i < c.publishIterations; i++ {
				...
				_ = publishInternal(&p, "foo", c.retryCount, "100", nil, &e)
			}
		})
	}
}
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			for i := 0; i < c.publishIterations; i++ {
				...
				want.Publishing.Body = toJSON(ziggurat.Event{Metadata: map[string]any{KeyRetryCount: retryCount}})
				p.On("Publish", want).Return(nil).Once()
				_ = publishInternal(&p, "foo", c.retryCount, "100", testBackoff, &e)
			}
		})
	}
}
